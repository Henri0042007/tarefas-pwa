...
export const useTasksStore = defineStore('tasks', () => {
  ...
  const filteredTasks = computed(() =>
    tasks.value.filter((t) => t.title.toLowerCase().includes(filterText.value.toLowerCase())),
  )
  const filteredPendingTasks = computed(() => filteredTasks.value.filter((t) => !t.done))
  const filteredCompletedTasks = computed(() => filteredTasks.value.filter((t) => t.done))
